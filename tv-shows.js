...
(function (window) {
    ...
    function searchTorrents(ep,movie,done,fail) {
        ...
        queries.forEach(function(query){
            var params={search:encodeURIComponent(query),movie:movie,clarification:true};
            try {
    ...
    function mount() {
        ...
        function loadShow(show){state.show=show;state.level='seasons';grid.innerHTML='<div class="tv-show-empty">Загрузка сезонов и выпусков…</div>';resolveShow(show).then(function(details){return apiGet(CONFIG.tvmazeUrl+'shows/'+encodeURIComponent(details.id)+'/episodes').then(function(eps){return {details:details,episodes:eps};});}).then(function(data){var d=data.details;state.movie={id:d.id,name:d.name||show.name,title:d.name||show.name,original_name:d.name||show.name,original_title:d.name||show.name,first_air_date:d.premiered||'',is_serial:true,number_of_seasons:d.runtime||0,source:'tvmaze'};state.episodes=data.episodes.map(function(ep){ep.showName=show.name;ep.torrent_queries=show.torrent_queries||show.queries||[];return ep;});showSeasons();}).catch(function(){grid.innerHTML='<div class="tv-show-empty">Не удалось получить сезоны и выпуски для «'+esc(show.name)+'».</div>';});}
