...
(function (window) {
    ...
    function torrentQuery(ep) {
        ...
        return [main].concat(aliases).join(' ');
    }
    ...
    function searchTorrents(ep,movie,done,fail) {
        ...
        window.Lampa.Parser.get({search:encodeURIComponent(torrentQuery(ep)),from_search:true},function(json){
            var results=[];
            (json||[]).forEach(function(group){ (group.Results||group.results||[]).forEach(function(item){ results.push(item); }); });
    ...
    function mount() {
        ...
        function loadShow(show){state.show=show;state.level='seasons';grid.innerHTML='<div class="tv-show-empty">Загрузка сезонов и выпусков…</div>';resolveShow(show).then(function(details){return apiGet(CONFIG.tvmazeUrl+'shows/'+encodeURIComponent(details.id)+'/episodes').then(function(eps){return {details:details,episodes:eps};});}).then(function(data){var d=data.details;state.movie={id:d.id,name:d.name||show.name,title:d.name||show.name,original_name:d.name||show.name,first_air_date:d.premiered||'',source:'tvmaze'};state.episodes=data.episodes.map(function(ep){ep.showName=show.name;ep.torrent_queries=show.torrent_queries||show.queries||[];return ep;});showSeasons();}).catch(function(){grid.innerHTML='<div class="tv-show-empty">Не удалось получить сезоны и выпуски для «'+esc(show.name)+'».</div>';});}
        ...
        function findTorrents(ep){state.level='torrents';header(ep.showName+' — '+ep.name,true);grid.innerHTML='<div class="tv-show-empty">Поиск торрентов: '+esc(torrentQuery(ep))+'…</div>';searchTorrents(ep,state.movie,function(items){cards(items,function(item){var size=item.size||item.Size||'';return '<div class="tv-show-card__name">'+esc(item.Title||item.title||'Раздача')+'</div><div class="tv-show-card__status">'+esc(size)+' · '+esc(item.Seeders||0)+' сидов · TorrServer</div>';},function(item){try{openTorrent(item,state.movie);}catch(e){grid.innerHTML='<div class="tv-show-empty">Не удалось открыть торрент через Lampa/TorrServer.</div>';}});},function(){grid.innerHTML='<div class="tv-show-empty">Не удалось выполнить поиск. Проверьте настройки парсера и TorrServer в Lampa.</div>';});}
